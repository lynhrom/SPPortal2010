...
function asynchContextMenuOnMouseClickHandler(uniqueID, isLTR, urlToMenuContents, menuBorderStyle, menuTableStyle, menuItemStyle, menuItemSelectedStyle, emptyMenuText, loadingImage) {
...
function buildAndDisplayMenu(menuID, iframeID, menuItemStyle, menuItemSelectedStyle, showMenu, onMenuAffordanceShowHandler) {
    ...
    if (showMenu == null || showMenu == true) {
        ...
        if (menuID.substring(0, 17) == "contextMenu_menu_") {
            menu.menuTag.style.zIndex = 999;
        }
        return;
    }
};
...
function asynchDoFormSubmit(url) {
    ...
    if (delimLocation >= 0) {
        var params = url.substring(delimLocation + 1, url.length);
        url = url.substring(0, delimLocation);
...
/* jQuery FlexSlider v2.5.0 | Copyright 2012 WooThemes | Tyler Smith */
...
/* jquery.matchHeight-min.js master | http://brm.io/jquery-match-height/ */
(function($) {
    /*
    *  internal 
    */
...
/* moment.js v2.10.3 | Tim Wood | momentjs.com */
...
(function(){
$.fn.gjSocialFeed=function(options){
  var defaults={
    facebook:{
			_id:"facebook",
      limit:15
    ...
    instagram:{
      ...
      limit:15
		...
		timeout:10000,
		errMsg:"Argh! Looks like connection is a little wonky, do check back again!",
    charLimit:undefined,
    charSub:"..."
  };
  ...
  handler = {
		_firstTab:true,
		init:function(o){
			var ft=this._firstTab;
			var id=o._id;
			var con = initTab(id,ft);
			if(id==fb._id){
				request("/Style Library/feed/SPSocialMediaFeed/Facebook.js",this.fb,con,ft);
			}else if(id==ins._id){
				request("/Style Library/feed/SPSocialMediaFeed/Instagram.js",this.instagram,con,ft);
			}else if(id==tw._id){
				request("/Style Library/feed/SPTwitterFeed/SPTwitterServlet.js",this.twitter,con,ft,"json");
			}
			if(ft){this._firstTab=false;}
		},
    fb:function(con,data,isFirstTab,errMsg){
			if(errMsg){initErrMsg(fb._id,con,isFirstTab,errMsg);return;}
			
      var list = data.data, pList=[];
      for(var i=0,len=list.length;i<len;i++){
        var p = list[i];
        var msg = p.message;
        if(!msg){
          //ignore fb feed without text (like profile picture updates / pure sharing links) &fields=created_time,from,message,picture,link
          continue;
        }else if(opts.charLimit){
          msg=msg.trunc(opts.charLimit,opts.charSub);
        }
        pList.push(new post(moment(p.created_time).fromNow(),p.from.name,msg,p.picture,p.link));
      }
      initPosts(fb._id,con,pList,isFirstTab,errMsg);
    },
    instagram:function(con,data,isFirstTab,errMsg){
			if(errMsg){initErrMsg(ins._id,con,isFirstTab,errMsg);return;}
			
     var list = data.data,pList=[];
     for(var i=0,len=list.length;i<len;i++){
      var p = list[i];
      pList.push(new post(moment(p.caption.created_time*1000).fromNow(),p.caption.from.username,null,p.images.low_resolution.url,p.link));
     }
     initPosts(ins._id,con,pList,isFirstTab,errMsg);
    },
		twitter:function(con,data,isFirstTab,errMsg){
			if(errMsg){initErrMsg(tw._id,con,isFirstTab,errMsg);return;}

			var list = data,pList=[],username, numOfTweet=0,twLimit=tw.limit;
			for(var i=0,len=list.length;i<len;i++){
				var p = list[i];
				if(p.in_reply_to_user_id){continue;}
				if(!username){username=p.user.screen_name;}
				if(numOfTweet>=twLimit){
					break;
				}
				var img=undefined,text=p.text;
				if(p.extended_entities && p.extended_entities.media){
					var media = p.extended_entities.media[0]; 
					img = media.media_url+":thumb";
					if(media.url){text=text.replace(media.url,"");}
				}
				/*moment(p.created_at)*/
				pList.push(new post(moment(p.created_at,"dd MMM DD HH:mm:ss ZZ YYYY").fromNow(),username,text,img,"http://twitter.com/"+username+"/status/"+p.id_str));
				numOfTweet++;
			}
			initPosts(tw._id,con,pList,isFirstTab,errMsg);
		}
  };
	
	// init the respective social containers
	self.addClass("gj-social").html($("<table class='tabs' cellpadding='0' cellspacing='0'></table>").append(tabs));  
	if(ordering){
		for(var i=0,len=ordering.length;i<len;i++){
			var order = ordering[i].toLowerCase();
			if(order=="facebook"){
				handler.init(fb);
			}else if(order=="instagram"){
				handler.init(ins);
			}else if(order=="twitter"){
				handler.init(tw);
			}
		}
	}
	var tabList=tabs.find("td"),tblList=self.find(".con");
	
  function initTab(tabName,isFirstTab){
		var tbl = $("<table cellpadding='0' cellspacing='0' class='"+tabName+" con' style='visibility:hidden'></table>");
		var tr = $("<tr></tr>");
		tr.append("<td class='flex-prev nav'>&nbsp;</td>");

		var fs = $("<div class='flexslider'></div>");
		var con = $("<ul class='slides'></ul>");
		fs.html(con);
		tr.append($("<td></td>").append(fs));
		
		tr.append("<td class='flex-next nav'>&nbsp;</td>");
    self.append(tbl.append(tr));
		
		var tab = $("<td"+(!isFirstTab?"":" class='active'")+">"+tabName.toUpperCase()+"</td>").click(function(e){
      selectTab($(this),tbl);
      return false;
    });
    tabs.append(tab);
  ...
  function initPosts(type,con,list,isFirstTab){
    for(var i=0,len=list.length;i<len;i++){
      var p=list[i];
      var n = $("<div class='post' data-url='"+p.url+"'></div>");
      n.append("<div class='by'><span class='icon'>&nbsp;</span><span>"+p.by+"</span></div>");
      n.append("<div class='from'>Posted "+p.fromNow+"</div>");
      if(p.text){
        n.append("<div class='text'>"+p.text+"</div>");
      }
      if(p.photo){
        n.append("<div class='photo'><img src='"+p.photo+"' /></div>");
      }
      if(p.url){
        n.click(function(e){
      ...
      con.append($("<li></li>").append(n));
    }

		var tbl = self.find("."+type);
		tbl.find('.flexslider').flexslider({
			animation: "slide",
			animationLoop: false,
			itemWidth: 199,
			slideshow:false,
			controlNav:false,
			customDirectionNav:tbl.find(".nav"),
			start:function(){
				if(!isFirstTab){tbl.css("display","none");}
				tbl.css("visibility","visible");
			}
		});
  }
	
	function initErrMsg(type,con,isFirstTab,errMsg){
		var tbl = self.find("."+type);
		if(!isFirstTab){tbl.css("display","none");}
		tbl.css("visibility","visible");
		con.html(errMsg);
		con.parent().removeClass("flexslider");
	};
	
	function request(url,process,con,isFirstTab,overwriteDataType){
		$.ajax({
			...
			success:function(data){
				process(con,data,isFirstTab);
			}
		}).fail(function(){
			process(con,null,isFirstTab,"<li style='padding:20px;'>"+errMsg+"</li>");
		});
	};

	function post(frNow,by,txt,photo,url){
		this.fromNow=frNow;
		this.by=by;
		this.text=txt;
		this.photo=photo;
...
String.prototype.trunc=function(limit,sub){
  if(this && this.length>limit){
    var s = this.slice(0,limit);
...
(function($){
	$.fn.gjMenuExt=function(options){
		// supporting classes and functions ------------------------------
		function MenuNode(li,parent,isBase){
			var self=this;
			this.li=li;
			this.a;
			this.url;
		...
		MenuNode.prototype.touchSupport=function(){
			var a=this.li.find("> a");
			if(!a.length){return;} // ignore if it doesn't contains <a>
			var self=this;
			this.a=a;
			this.url=a.attr("href");
			this.target=a.attr("target");
			a.attr("href","#").click(function(){
				if(!self.touchCnt){
					if(self.isBase){
						var page = $("html");
						page.off("click.menuNode");
						page.on("click.menuNode", function(d){
							n=d.target;
							while(n){
								// continue to transverse until <html>
								if ($(n).hasClass("gjm-menu")){return;}
								n = n.parentNode;
							}
							resetAllTouchCount(self.parent);
						});
					}
					self.parent.resetChildrenTouchCount();
					self.li.addClass("active");
					self.touchCnt=1;
				}else{
		...
		MenuNode.prototype.delaySupport=function(){
		
			var self=this;
			var li=this.li;
			li.on("mouseenter",function(){
				setTimeout(function(){li.addClass("hover");},_delay);
			}).on("mouseleave",function(e){
				setTimeout(function(){
					if(cursorWithinMenu){
						li.removeClass("hover");
					}else{
						self.delayEvent=setTimeout(function(){
							li.removeClass("hover");
						},delay);
					}
				},_delay);	
		...
		MenuNode.prototype.addChild=function(li,isBase){
		
			var mn = new MenuNode(li,this,isBase);
			if(touchMode && hasChild(li)){
				mn.touchSupport();
				this.children.push(mn);
			}else{
				mn.delaySupport();
				this.children.push(mn);
			}
		...
		MenuNode.prototype.resetChildrenTouchCount=function(){
			var cList = this.children;
			for(var i=0,len=cList.length;i<len;i++){
				var child=cList[i];
				child.li.removeClass("active");
				child.touchCnt=0;
				child.resetChildrenTouchCount();
			}
		};
		MenuNode.prototype.clearDelayTimeout=function(){
			if(this.delayEvent){
				var li=this.li;
				window.clearTimeout(this.delayEvent);
				this.delayEvent=null;
				li.removeClass("hover").addClass("active");
				setTimeout(function(){li.removeClass("active");},_delay);				
			}
		};
		function hasChild(li){
			return li.has("li").length;
		};
		function resetAllTouchCount(mn){
			for(var i=0,len=mn.children.length;i<len;i++){
				var child = mn.children[i];
				child.li.removeClass("active");
				child.touchCnt=0;
				resetAllTouchCount(child);
			}
		};
		function resetAllDelayEvent(mn){
			for(var i=0,len=mn.children.length;i<len;i++){
				var child = mn.children[i];
				var title = child.li.find("> a").html();
				child.clearDelayTimeout();
				resetAllDelayEvent(child);
			}
		};
		function transverse(li,parentNode){
			li.find("> ul > li").each(function(){
				var node=$(this);			
				var mn = parentNode.addChild(node,false);
				transverse(node,mn);			
			});
		};
		...
		var opts=$.extend(true,defaults,options),
		menu=this,
		touchMode=(opts.supportTouch && ('ontouchstart' in window || navigator.MaxTouchPoints || navigator.msMaxTouchPoints))?true:false,
		delay=opts.delay,
		_delay=0,
		cursorWithinMenu=true,
		mainNode = new MenuNode();
		
		// locate level 1 base nodes
		menu.find("> li").each(function(){
			var base = $(this);
			var mn = mainNode.addChild(base,true);
			transverse(base,mn);
		});
		menu.addClass("gjm-menu");
		
		if(!touchMode && delay){
			menu.on("mouseenter",function(){
				resetAllDelayEvent(mainNode);
				cursorWithinMenu=true;
			}).on("mouseleave",function(){cursorWithinMenu=false;});
		}
	};
